...
3. Use Razorpay Standard Checkout for new purchases, retaining existing Payment Links. Verify the checkout signature and fetch captured payments server-side against order identity, amount, currency and stored checkout terms. Signed webhooks and authenticated reconciliation use the same settlement code. Never credit on a browser redirect or a client payment ID alone.
...
Local Razorpay test checkout is enabled. Production checkout still requires the saved enabled switch, backend secrets, merchant identity, support and policy URLs. Register `payment.captured`, `order.paid`, payment refund and dispute events on the existing signed webhook endpoint; retain Payment Link events for existing links. Reconcile from Wallet or Admin when notification delivery is delayed. Deployment should schedule `python -m backend.reconcile_payments --limit 50` every five minutes. It rotates through stored purchases and emits a nonzero status for unresolved checks; it never initiates a charge. The host scheduler itself has not been configured here. See [Standard Checkout setup and testing](RAZORPAY_CHECKOUT.md).
...
Standard Checkout now has configured local test credentials and an enabled saved switch. A test API order was verified without a charge; the browser capture/refund cycle and production activation steps remain documented in [the checkout guide](RAZORPAY_CHECKOUT.md). Deep Researched Stock Analysis remains a future feature.
