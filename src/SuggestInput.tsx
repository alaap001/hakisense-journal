...
export function SuggestInput({options,defaultValue,id,...props}:Omit<InputHTMLAttributes<HTMLInputElement>,'value'|'onChange'|'list'> & {options:string[]}) {
  ...
  const [position,setPosition]=useState({left:0,top:0,width:240,maxHeight:260});
  ...
  useLayoutEffect(()=>{
    ...
    setPosition({left:Math.max(12,Math.min(rect.left,window.innerWidth-width-12)),top:above?Math.max(12,rect.top-Math.min(height,maxHeight)-6):rect.bottom+6,width,maxHeight});
    ...
    return()=>{window.removeEventListener('scroll',close,true);window.removeEventListener('resize',close);if(el?.hidePopover&&el.matches(':popover-open'))el.hidePopover();};
  },[visible]);
  useLayoutEffect(()=>{if(active>=0)menu.current?.querySelector(`[data-index="${active}"]`)?.scrollIntoView({block:'nearest'});},[active]);
  const choose=(v:string)=>{setValue(v);setOpen(false);setActive(-1);input.current?.focus();};
