...
export function HelpPage({publicPage=false}:{publicPage?:boolean}){
  ...
  return <><PageTitle eyebrow="A LITTLE HELP, WHEN YOU NEED IT" title="Help & customer care" description="Find an answer, manage your account, or get in touch."/>{!publicPage&&<TourLauncher/>}<div className="help-shortcuts">{[[BookOpen,'Journal & imports','Your trades, in one place.','/import'],[CreditCard,'Credits & payments','Recharges, usage and payment history.','/billing'],[ShieldCheck,'Account & security','Email, password and sessions.','/settings/security']].map(([Icon,title,text,path]:any)=><Link key={path} to={destination(path)}><Icon size={22}/><strong>{title}</strong><span>{text}</span><ArrowUpRight size={16}/></Link>)}</div>
    ...
    <div className="help-policies">{[['privacy','Privacy policy','/privacy'],['terms','Terms of service','/terms'],['refunds','Refund policy','/refunds']].map(([key,label,path])=><Link to={path} key={key}>{label} <ArrowUpRight size={13}/></Link>)}</div>
  </>;
}

export default function PublicHelp(){
  useEffect(()=>{document.title='Help & customer care · HakiSense';},[]);
  return <div className="public-help"><header><Link to="/" className="public-help-brand"><span className="brand-logo">H</span>HakiSense</Link><nav aria-label="Help navigation"><Link to="/pricing">Pricing</Link><Link className="button primary" to="/login">Open my journal <ArrowUpRight size={16}/></Link></nav></header><main><HelpPage publicPage/></main><footer><Link to="/">← Back to HakiSense</Link><div style={{display:'flex',gap:20}}><Link to="/terms">Terms</Link><Link to="/privacy">Privacy</Link><Link to="/refunds">Refunds</Link></div><span>Made for Indian traders · INR / IST</span></footer></div>;
}
