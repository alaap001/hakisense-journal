...
export type Catalog = {ai_pricing:Pick<AIPricing,'min_credits'|'max_credits'|'description'>;reference_credit_paise:number;billing_model:string;packs:CreditPack[];monthly_free_credits:number;playbook_creation_credits:number;advanced_credit_multiplier:number;brand_name:string;upcoming_features:Record<string,string>;tasks:{code:string;name:string;credits:number;advanced_credits:number;enabled:boolean}[];features:Record<string,string>;credit_policy:string;tax_policy:string;legal:Record<string,string>};
