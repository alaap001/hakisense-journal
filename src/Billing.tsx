...
import { loadRazorpay,openCheckout } from './razorpay';
...
export function BillingPage(){
  ...
  async function recharge(code:string){
    ...
    setBusy(code);setNotice('');try{
      await loadRazorpay();
      let key=sessionStorage.getItem(storageKey);if(!key){key=crypto.randomUUID();sessionStorage.setItem(storageKey,key);}
      const result=await api('/billing/create-order',{method:'POST',body:JSON.stringify({pack_code:code,expected_amount_paise:terms.amount_paise,expected_credits:terms.credits}),headers:{'Idempotency-Key':key}});
      if(result.url){window.location.assign(result.url);return;}
      refresh();
      const payment=await openCheckout(result);
      if(!payment){setNotice('Payment cancelled. Your pending recharge is saved so you can continue when ready.');return;}
      setNotice('Confirming your payment…');
      try{
        const verified=await post('/billing/verify-payment',payment);
        if(verified.success){sessionStorage.removeItem(storageKey);setNotice('Payment verified. Your credits have been added.');notify('Recharge complete.');}
        else setNotice(verified.message||'Payment is awaiting confirmation. Use Refresh payments shortly.');
      }catch(e){setNotice('Payment confirmation is incomplete. Use Refresh payments before starting another recharge.');throw e;}
      finally{refresh();}
  ...
  return <><PageTitle eyebrow="PAY AS YOU GO" title="Your credit wallet" description="Your tools stay unlocked. Recharge when you want to explore further." actions={<Button disabled={!!busy} onClick={()=>void reconcile()}><RefreshCw size={16} className={busy==='refresh'?'spin':''}/>Refresh payments</Button>}/>
    {catalog.payment_mode==='test'&&<div className="info-bar section-gap" role="status">Razorpay test mode: no real money is charged. {catalog.test_checkout_admin_only?'Checkout is available only to administrators. ':''}Test purchases add credits to this account; use a dedicated test account.</div>}
