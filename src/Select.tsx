...
export function Select({children, value, defaultValue, onChange, className = '', id, disabled, ...props}: SelectHTMLAttributes<HTMLSelectElement>) {
  ...
  useLayoutEffect(() => {
    ...
    return () => {if (el?.hidePopover && el.matches(':popover-open')) el.hidePopover();};
  }, [open]);
  useEffect(() => {
    if (!open) return;
  ...
  function keys(e: KeyboardEvent<HTMLElement>) {
    ...
    if (e.key === 'Tab') {if (e.target === search.current) trigger.current?.focus(); close(); return;}
  ...
  return <span className={`themed-select ${className}`}>
    <select {...props} ref={native} id={`${triggerId}-native`} disabled={disabled} value={value} defaultValue={defaultValue} tabIndex={-1} aria-hidden="true" className="select-native" onInvalid={e => {e.preventDefault(); trigger.current?.focus(); show();}} onChange={e => {setInternal(e.target.value); onChange?.(e);}}>{children}</select>
    ...
    {open && createPortal(<div ref={menu} popover="manual" className="select-menu" style={position} onKeyDown={keys}>
