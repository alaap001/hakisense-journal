...
export function AuthBoundary({children}:{children:(session:Session)=>ReactNode}){
  ...
  if(!session||recovering)return <AuthScreen client={client} config={config} recovering={recovering} onRecovered={()=>{setRecovering(false);history.replaceState({},'', '/');}}/>;
...
function AuthScreen({client,config,recovering,onRecovered}:{client:SupabaseClient;config:PublicConfig;recovering:boolean;onRecovered:()=>void}){
  const [mode,setMode]=useState<'login'|'signup'|'forgot'>('login'),[busy,setBusy]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState('');
