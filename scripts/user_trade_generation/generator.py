...
def generate_trades(account_id, count=100, seed=42):
    random.seed(seed)
    trades = []
    
    if count > 250:
        start_date = datetime(2026, 1, 5, 9, 15, tzinfo=IST)
    else:
        start_date = datetime(2026, 6, 1, 9, 15, tzinfo=IST)
    end_date = datetime(2026, 9, 13, 15, 0, tzinfo=IST)
    total_days = (end_date - start_date).days

    open_threshold = count - max(2, int(count * 0.05))
    partial_threshold = open_threshold - max(2, int(count * 0.03))
    ...
    for i in range(count):
        ...
        if cat == 'Equity':
            ...
            if side == 'Long':
                ...
                exit_price = round(entry_price + p_diff * random.uniform(0.8, 2.2), 2) if is_win else round(entry_price - p_diff * random.uniform(0.7, 1.1), 2)
            else:
                stop_loss = round(entry_price + p_diff, 2)
                target_price = round(entry_price - p_diff * 2.0, 2)
                exit_price = round(entry_price - p_diff * random.uniform(0.8, 2.2), 2) if is_win else round(entry_price + p_diff * random.uniform(0.7, 1.1), 2)
            ...
            lots = random.randint(2, 4) if status == 'Partial' else random.randint(1, 4)
            quantity = lots * lsize
            lot_size = lsize
            asset_type = 'Options'
            exchange = 'NSE'
            ...
            if side == 'Long':
                ...
                target_price = round(entry_price + p_diff * 2.0, 2)
                exit_price = round(entry_price + p_diff * random.uniform(1.0, 2.5), 2) if is_win else round(max(5.0, entry_price - p_diff * random.uniform(0.8, 1.0)), 2)
            else:
                stop_loss = round(entry_price + p_diff, 2)
                target_price = round(max(5.0, entry_price - p_diff * 2.0), 2)
                exit_price = round(max(5.0, entry_price - p_diff * random.uniform(1.0, 2.5)), 2) if is_win else round(entry_price + p_diff * random.uniform(0.8, 1.0), 2)
            ...
            lots = 2 if status == 'Partial' else random.randint(1, 2)
            quantity = lots * lsize
            lot_size = lsize
            asset_type = 'Futures'
            exchange = 'NSE'
            ...
            if side == 'Long':
                stop_loss = round(entry_price - p_diff, 2)
                target_price = round(entry_price + p_diff * 2.0, 2)
                exit_price = round(entry_price + p_diff * random.uniform(1.0, 2.2), 2) if is_win else round(entry_price - p_diff * random.uniform(0.8, 1.1), 2)
            else:
                stop_loss = round(entry_price + p_diff, 2)
                target_price = round(entry_price - p_diff * 2.0, 2)
                exit_price = round(entry_price - p_diff * random.uniform(1.0, 2.2), 2) if is_win else round(entry_price + p_diff * random.uniform(0.8, 1.1), 2)
            ...
            if side == 'Long':
                stop_loss = round(entry_price - p_diff, 2)
                target_price = round(entry_price + p_diff * 2.0, 2)
                exit_price = round(entry_price + p_diff * random.uniform(0.8, 2.0), 2) if is_win else round(entry_price - p_diff * random.uniform(0.8, 1.0), 2)
            else:
                stop_loss = round(entry_price + p_diff, 2)
                target_price = round(entry_price - p_diff * 2.0, 2)
                exit_price = round(entry_price - p_diff * random.uniform(0.8, 2.0), 2) if is_win else round(entry_price + p_diff * random.uniform(0.8, 1.0), 2)
        ...
        if status == 'Closed':
            ...
            if lot_size:
                closed_quantity = float(max(1, (quantity // lot_size) // 2) * lot_size)
                if closed_quantity >= quantity:
                    closed_quantity = float(quantity - lot_size)
            else:
